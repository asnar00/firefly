...
export class Graph {
    ...
    assignNodeRec(doNodes: any[]) {     // doNode contains {node, iCol, from, forward, indent}
        while(doNodes.length > 0) {
            let d = doNodes[0];
            doNodes.shift();
            let node = d.node; let iCol = d.iCol; let fromNode = d.fromNode; let forward = d.forward; let indent = d.indent;
