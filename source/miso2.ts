...
class Connection {
    ...
    constructor(card: HTMLElement, button: HTMLElement, parentCard: HTMLElement) {
        this.card = card; this.button = button; this.parentCard = parentCard;
    }
};

// organised into columns; main is column 0, things it calls are column 1, and so on.
let s_connections : Connection[][] = []; 

function arrangeAll() {
    let xMin = 45;
    for(let level = 0; level < s_connections.length; level++) {

        for(let c of s_connections[level]) {
        }
    }
}